    ...
    <div class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <nav class="flex items-center space-x-2 text-sm">
          ...
          <span class="text-gray-900 dark:text-white font-medium">{{ props.effect?.name }}</span>
        </nav>
      </div>
    </div>
        ...
        <div class="text-center mb-8">
          ...
          <div class="flex flex-wrap items-center justify-center gap-4 text-sm text-gray-500 dark:text-gray-400">
            <div class="flex items-center">
              ...
              {{ props.effect?.author || 'MiniMax Agent' }}
            </div>
            <div class="flex items-center">
              <svg class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3a2 2 0 012-2h4a2 2 0 012 2v4m-6 9l6-6m-6 0l6 6" />
              </svg>
              {{ props.effect?.createdAt ? formatDate(props.effect.createdAt) : '' }}
            </div>
            <div class="flex items-center">
              <svg class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
              </svg>
              {{ props.effect?.likes || 0 }} 点赞
            </div>
            <div class="flex items-center">
              <span class="px-2 py-1 text-xs font-medium rounded-full"
                    :class="{
                      'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200': props.effect?.difficulty === 'easy',
                      'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200': props.effect?.difficulty === 'medium',
                      'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200': props.effect?.difficulty === 'hard'
                    }">
                {{ props.effect?.difficulty === 'easy' ? '简单' : props.effect?.difficulty === 'medium' ? '中等' : '困难' }}
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl overflow-hidden">
          <div class="p-8">
            ...
            <div class="min-h-96 bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-800 dark:to-gray-900 rounded-xl p-8 flex items-center justify-center">
              <div v-if="props.effect?.demoComponent" class="w-full">
                <component :is="props.effect.demoComponent" />
              </div>
    ...
    <div v-if="showCode" class="bg-white dark:bg-gray-800">
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="bg-gray-900 rounded-2xl overflow-hidden">
          ...
          <!-- 代码内容 -->
          <div class="relative">
            <pre class="p-6 overflow-x-auto"><code class="text-sm text-gray-300">{{ activeTab === 'html' ? props.effect?.html : props.effect?.css }}</code></pre>
            <button
              @click="copyToClipboard(activeTab === 'html' ? props.effect?.html || '' : props.effect?.css || '')"
    ...
    <div class="bg-gray-50 dark:bg-gray-900">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex flex-wrap gap-2">
          <span v-for="tag in props.effect?.tags || []" :key="tag" 
